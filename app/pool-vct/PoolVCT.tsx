...
export default function PoolVCT() {
  ...
  const [ready,setReady]=useState(false),[entered,setEntered]=useState(false),[error,setError]=useState('');
  ...
  const [status,setStatus]=useState<Status>({x:0,z:0,rooms:9,discovered:1,fps:0,vct:true,impacts:0,caustics:true,hint:'',filter:1,held:'',throws:0,grabs:0,height:1.5,climbing:false,slides:0,charge:0});
  ...
  useEffect(()=>{const onLock=()=>setEntered(!!document.pointerLockElement);document.addEventListener('pointerlockchange',onLock);return()=>document.removeEventListener('pointerlockchange',onLock);},[]);
  return <main className="vct-page">
    ...
    {!entered&&<section className="vct-menu">
      ...
      <p className="vct-keys">W A S D 移动 · Shift 快走 · 空格跳跃<br/>左键拖动 · E 拿起 / 攀爬 · 右键按住蓄力投掷<br/>滚轮调整距离 · 梯子上 W / S 攀爬<br/>Esc 暂停 · C 焦散 · V 间接光 · R 起点</p>
      ...
      <nav className="vct-destinations" aria-label="前往房间">
        ...
        {([['旧泳池 · 阴暗',0,0],['高拱门大厅 · 明亮',0,-1],['连通浴池 · 明亮',-1,0],['跳台高廊 · 明亮',1,0],['夜间浴场 · 阴暗',0,1]] as const).map(([label,x,z])=><button key={label} disabled={!ready} onClick={()=>engine.current?.visitRoom(x,z)}>{label}</button>)}
      </nav>
    ...
    <footer className="vct-footer"><div><span>SECTOR {String(status.x).padStart(2,'0')} / {String(status.z).padStart(2,'0')}</span><small>已探索 {status.discovered} 个区域 · {status.rooms} 个驻留区块</small></div><div className="vct-tech">{status.vct?'体素锥追踪 GI':'GI 对照：关闭'} · {status.caustics?'水光焦散':'焦散关闭'}<small>{status.fps} FPS · SEED {seed} · 水面交互 {status.impacts}</small></div><time aria-label="摄像机本地时间">{cameraTime}</time></footer>
  </main>;
}
